...
export default <TestSuite>{
  ...
  'getFee - high load_factor with custom maxFeeBRT': async (api, address) => {
    // Ensure that overriding with high maxFeeBRT of '51540' causes no errors.
    // (fee will actually be 51539.607552)
    api._maxFeeBRT = '51540'
    api.connection.request({
      command: 'config',
      data: {highLoadFactor: true}
    })
